...
class SpreadServiceIntegrationTest {
  ...
  private static final ProgressListener NOOP_LISTENER =
  ...
  void createFixtures() throws IOException {
    fourPagePdf =
        createTestPdf(
            tempDir.resolve("4pages.pdf"),
            4,
            PDRectangle.A4.getWidth(),
            PDRectangle.A4.getHeight());
    fivePagePdf =
        createTestPdf(
            tempDir.resolve("5pages.pdf"),
            5,
            PDRectangle.A4.getWidth(),
            PDRectangle.A4.getHeight());
    singlePagePdf =
        createTestPdf(
            tempDir.resolve("1page.pdf"), 1, PDRectangle.A4.getWidth(), PDRectangle.A4.getHeight());
  }

  private Path createTestPdf(Path path, int pageCount, float width, float height)
      throws IOException {
